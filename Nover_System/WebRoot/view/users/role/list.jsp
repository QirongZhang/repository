...
function deleteRole(role_id){
	...
	if (flag == true){
		var deleteURL = "<%=path%>/RoleAction!deleteRole?role_id="+role_id+"";
	...
	table=$('#resultfile_datatable').DataTable(//创建一个Datatable
	        {
	        			"lengthMenu": [ 6, 15, 20],     
	                    ajax : {//通过ajax访问后台获取数据
	                        "url": "<%=path%>/RoleAction!getAllResultFiles",//后台地址
	                       ...
	                       {//列渲染，可以添加一些操作等
	                        ...
	                        render : function(data, type, row) {//渲染函数                
	                        	var id =  row.role_id;
	                       		...
	                       		//alert(id);
	                            var html = "&nbsp;<a class='btn btn-info btn-sm' id='btn-edit' onclick=\"jumpimportfilepage(\'<%=path%>/RoleAction!editRole?role_id= "+id+"\');\" >修改</a>"
	                    ...
	                    initComplete : function() {//表格完成时运行函数                      
	                        $("#mytool").append('<button type="button" class="btn btn-warning btn-sm" id="importfilebutton" onclick="jumpimportfilepage(\'<%=path%>/RoleAction!showAddJsp\');">添加</button>');//这里在表格的上面加了一个添加标签
	                    }
	                });
...
<style type="text/css">
	.role-list{width : 95%;margin: 0 auto;}
...
<s:debug></s:debug>
	<div class="role-list">
		<div class="table text-center" style="margin-top: 20px;">
			...
			<s:fielderror name="deleteMessage" cssStyle="color:red;margin-left:15%;"></s:fielderror>
		</div>
		<!-- table结束 -->
	</div>
	<!-- 模态框（Modal） -->
	...
	<div>
		<div class="btn-group">
			<a href="RoleAction!addRole" class="btn btn-default"><span class="glyphicon glyphicon-plus"></span>增加</a>
		</div>
		<div>
			<label>角色名称</label><input name="search_name" id= "search_name">
			<a class="btn btn-default" id="btn"><span class="glyphicon glyphicon-search"></span>查询</a>
		</div>
		<div class="table">
				  ...
				  <tbody>
				  	<s:iterator value="roleList" status="myStatus">
				  		<tr>
							...
							<td><input type="checkbox" name="role_check" value="<s:property value="role_id"/>"> </td>
							<td><s:property value="role_id"/></td>
							<td><s:property value="role_name"/></td>
							<td><s:property value="role_remark"/></td>
							<td>
							<a href="<%=path%>/RoleAction!editRole?role_id=<s:property value="role_id"/>"  class="btn btn-default">
							<span class="glyphicon glyphicon-pencil"></span>修改</a>
							<s:if test="flag == true">
								<a href="javascript:void(0);" onclick="deleteRole('<s:property value="role_id"/>');" class="btn btn-default">
